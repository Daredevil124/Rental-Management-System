...
import { Truck, MapPin, QrCode, CheckCircle, AlertCircle, X } from 'lucide-react';
...
const PickupReturnBoard = () => {
  ...
  const handleProcessReturn = () => {
    ...
    alert('Return processed! Penalty deducted and remaining deposit refunded.');
  };
            ...
            {!returnedOrders.includes('ORD-002') && (
              <div className="task-card glass-panel border-warning">
                ...
                <div className="task-body">
                  ...
                  <div className="return-inspection mt-2 bg-warning/10 p-2 rounded flex items-start gap-2">
                    <AlertCircle size={16} className="text-warning mt-0.5 shrink-0" />
                    <div>
                      <p className="text-warning text-sm font-semibold">2 Days Late</p>
                      <p className="text-gray-400 text-xs">Late fee applies (₹500/day)</p>
                    </div>
                  </div>
                  <div className="task-action-row mt-4">
                    <button className="btn-secondary text-danger">Report Damage</button>
                    <button className="btn-primary" onClick={() => setProcessingReturn({ id: 'ORD-002', deposit: 3000, lateDays: 2, feePerDay: 500 })}>Process Return</button>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="glass-panel p-6 w-full max-w-md animate-fade-in border border-warning">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold text-white">Process Return</h2>
            ...
            <div className="space-y-4">
              <div className="bg-gray-900 p-4 rounded-lg border border-gray-700">
                ...
                <div className="flex justify-between mb-2 text-warning">
                  <span>Late Penalty ({processingReturn.lateDays} days @ ₹{processingReturn.feePerDay}):</span>
                  <span>- ₹{processingReturn.lateDays * processingReturn.feePerDay}</span>
                </div>
                ...
                <div className="flex justify-between text-lg font-bold">
                  ...
                  <span className="text-success">₹{processingReturn.deposit - (processingReturn.lateDays * processingReturn.feePerDay)}</span>
                </div>
              </div>

              <button className="btn-primary w-full justify-center bg-success hover:bg-success/80 border-success" onClick={handleProcessReturn}>
